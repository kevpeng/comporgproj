...
class ALU {
  ...
  public:
    ...
    // Set operand1 equal to the argument string - input format must be a 32
    // bits binary string
    void setOperand1(string operand);

    // Set operand2 equal to the argument string - input format must be a 32 bits
    // binary string
    ...
    // Returns the output - format is 32 bits binary string except when
    // comparing - then it would be string "true" if they are equal, "false"
    // otherwise
    string getOutput();

    // returns string "true" if two operands are equal, string "false" otherwise
    string isEqual();
